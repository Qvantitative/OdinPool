...
const AnalyticsPage = () => {
  ...
  const handleSearchSubmit = async (e) => {
    ...
    if (searchType === 'Transaction ID') {
      ...
      try {
        const response = await fetch(`http://143.198.17.64:3001/api/ord/block/${searchInput.trim()}`);
      ...
      try {
        const response = await fetch(`http://143.198.17.64:3001/api/ord/address/${searchInput.trim()}`);
