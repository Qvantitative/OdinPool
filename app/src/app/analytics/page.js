...
const AnalyticsPage = () => {
  ...
  const fetchBlockData = async () => {
    try {
      ...
      const response = await fetch(`https://143.198.17.64:3001/api/blocks?limit=${maxBlocks}`);
  ...
  const handleSearchSubmit = async (e) => {
    ...
    if (searchType === 'Transaction ID') {
      ...
      try {
        const response = await fetch(`https://143.198.17.64:3001/api/ord/block/${searchInput.trim()}`);
      ...
      try {
        const response = await fetch(`https://143.198.17.64:3001/api/ord/address/${searchInput.trim()}`);
