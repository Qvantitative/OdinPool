...
const TransactionDetails = ({ transactionId }) => {
  ...
  useEffect(() => {
    const fetchTransactionDetails = async () => {
      ...
      try {
        const response = await fetch(`https://143.198.17.64:3001/api/transactions/${transactionId}`);
        if (!response.ok) {
        ...
        const inscriptionResponse = await fetch(`https://143.198.17.64:3001/api/ord/inscription/${inscriptionId}`);
  ...
  const handleOpReturnClick = async (index) => {
    if (expandedOpReturn === index) {
      ...
      try {
        const response = await fetch(`https://68.9.235.71:3001/api/rune/${transactionId}`);
        if (!response.ok) {
                        ...
                        <div>
                          ...
                          {inscriptionData.content_type.startsWith('image/') && (
                            <div className="mt-2 flex justify-center">
                              <img
                                src={`https://68.9.235.71:3000/content/${inscriptionData.id}`}
