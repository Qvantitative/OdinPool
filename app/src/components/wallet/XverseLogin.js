  ...
  const handleConnect = async () => {
    try {
      ...
      if (response.status === 'success') {
        console.log("Wallet connected successfully:", response.accounts);
        sessionStorage.setItem('accountData', JSON.stringify(response.result));
        setWalletConnected(true);
        router.push('/dashboard'); // Proceed to dashboard
      } else {
        console.error("Failed to connect to the wallet:", response);
        alert('Failed to connect to the wallet');
      }
    } catch (err) {
      console.error("Error during wallet connection process:", err);
      alert('Something went wrong during the wallet connection process');
    }
  };
  ...
  return (
    <div>
      <button
        ...
        className="text-white p-2 rounded-md shadow-sm hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors duration-200 flex items-center justify-center"
      >
...
export default dynamic(() => Promise.resolve(XverseLogin), { ssr: false });
